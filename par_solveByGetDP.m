function par_solveByGetDP(tmpDir,P,current,sigma,indUse,uniTag,LFtag)
...
% June 2020 Parrallel processing by Alejandro Albizu
...
fid = fopen(fullfile(tmpDir,[baseFilename '_' uniTag '.pro']),'w');
...
cmd = [solverPath ' "' tmpDir filesep baseFilename '_' uniTag '.pro" -solve EleSta_v -msh "' tmpDir filesep baseFilename '_' uniTag '_ready.msh" -pos Map'];
try
    status = system(cmd);
catch
end
...
%     delete([tmpDir filesep baseFilename '_' uniTag '.pre']);
%     delete([tmpDir filesep baseFilename '_' uniTag '.res']);
    movefile(fullfile(tmpDir,[baseFilename '_' uniTag '_e' LFtag '.pos']),fullfile(dirname,[baseFilename '_' uniTag '_e' LFtag '.pos']))
    rmdir(tmpDir,'s')
end