function [node,elem,face]=cgalv2m(s,vol,opt,maxvol)
%
...
saveinr(vol,fullfile(pwd,'mesh',strcat('pre',num2str(s),'_cgalmesh.inr')));
deletemeshfile(fullfile(pwd,'mesh',strcat('post',num2str(s),'_cgalmesh.mesh')));
...
cmd=sprintf(['"%s%s" "%s" "%s" %f %f %f %f ' format_maxvol ' %d'],fullfile(pwd,'lib','iso2mesh','bin','cgalmesh'),exesuff,...
    fullfile(pwd,'mesh',strcat('pre',num2str(s),'_cgalmesh.inr')),fullfile(pwd,'mesh',strcat('post',num2str(s),'_cgalmesh.mesh')),ang,ssize,...
    approx,reratio,maxvol,randseed);
system(cmd);
if(~exist(fullfile(pwd,'mesh',strcat('post',num2str(s),'_cgalmesh.mesh')),'file'))
    error(['output file was not found, failure was encountered when running command: \n',cmd]);
end
[node,elem,face]=readmedit(fullfile(pwd,'mesh',strcat('post',num2str(s),'_cgalmesh.mesh')));
