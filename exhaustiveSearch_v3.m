...
function exhaustiveSearch_v3(sub,e)
...
weights = abs(weights(brain)); % shrink weights to size of group brain
...
%     J_sim(brain) = J_sim(brain)+(abs(min(J_sim(brain)))+1); % Standardize
    ...
    % log-likelihood
    LL(i,:) = [exp(-sum((((J_sim' - g_mean').*fprec').^2 ./ (g_std'.^2 + 1)),2)), ... % 
        exp(-sum((((-J_sim' - g_mean').*fprec').^2 ./ (g_std'.^2 + 1)),2))]; 
end
[maxcol,~] = max(LL);
