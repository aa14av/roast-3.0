...
% Load SVM Weights
load(fullfile(outDir,'a_I_MNI.mat'),'activ','Mdl'); % Changed from v1 20211202
POSJ = mean(J_RinterpN(:,activ ~= 0),2,'omitnan');
...
for s = 1:length(subs)
    ...
    w = repmat(activ,1,nd)./3 * (p/100); w=w'; % Repeat along # dims
...
%         N(ii) = exp(-sum( ( (x - r_mean).^2  ./ (1 + r_std.^2) ) .* w,'omitnan'));
        clear x;
    end; [LL(s), indLL] = max(N); I_opt(s,:) = [I(indLL),I(indLL)] .* [1 -1];
    %=================================================

    % Compute Intensity from paper
    %=================================================
%    I_opt(s,:) = round([2 -2] .* (1 + ( median(POSJ) - median(fJbrain(activ ~= 0),'omitnan') ) / median(fJbrain(activ ~= 0),'omitnan')),1); % IPrec from equation
    %=================================================
end