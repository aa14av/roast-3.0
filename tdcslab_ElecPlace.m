function hdrInfo = tdcslab_ElecPlace(day,P1,P2,T2,elecNeeded,options,uniTag)
...
if isempty(T2)
    baseFilenameRSPD = [baseFilename '_T1orT2'];
else
...
if ~isempty(indC)
    if ~isempty(day)
        fid = fopen([dirname filesep baseFilename '_day' day '_customLocations.txt']);
    else
        fid = fopen([dirname filesep baseFilename '_avg_customLocations.txt']);
    end
...
save_untouch_nii(template,[dirname filesep baseFilename '_mask_elec.nii']);
...
save_untouch_nii(template,[dirname filesep baseFilename '_mask_gel.nii']);
