...
function hiperROAST_target_PAD_sh(rootDir,subname,idx)
...
s = Nelec-(ceil(idx/Nelec)*Nelec-idx);
eTag = elecName{Nelec-(ceil(idx/Nelec)*Nelec-idx)}; m=1;
...
while ~exist(fullfile(dirname,[baseFilename '_' eTag '_roastResult.mat']),'file')
    if m > 5; disp([subname ': ITERATION LIMIT REACHED FOR ' eTag]); break; end % QUIT AFTER 5 ITERATIONS
    try; hiperskipseg(s,[],subj,'elec',elecName([Nelec-(ceil(idx/Nelec)*Nelec-idx),end])'); end
