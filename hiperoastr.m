function status = hiperoastr(s,subj,montage)
...
doResamp = 0;
voxsize = [1 1 1]; % Voxel Size MUST BE ISOTROPIC
...
% end
%
if contains(subj,'FS6.0_')
    subname = subj(strfind(subj,'FS6.0_')+6:strfind(subj,'_ses01')-1);
elseif contains(subj,'MBAR')
    subname = subj(strfind(subj,'sub-MBAR'):strfind(subj,'sub-MBAR')+12);
else
    subname = subj(strfind(subj,'sub-'):strfind(subj,'sub-')+8);
end
...
if ~exist('conductivities','var')
    ...
    %                            'skin',0.465,'air',2.5e-14,'gel',0.3,'electrode',5.9e7); % literature values default
...
if ~strcmpi(subj,'example/nyhead.nii') % only when it's not NY head
    ...
    if doResamp
        subjRS = resampToOneMM(subj,voxsize);
    else
        subjRS = subj;
    end
...
if ~strcmp(baseFilename,'nyhead')
    ...
    if (isempty(T2) && ~exist([dirname filesep 'c1' baseFilenameRSPD '_T1orT2.nii'],'file')) ||...
        ...
        disp('======================================================')
        disp(strcat('- MATLAB Worker No',num2str(s),' for',subname))
        logfile(logname,[subname '(' num2str(s) ') BEGAN start_seg']);
        try
            segstart = tic;
            start_seg(subjRSPD,T2);
            segend = toc(segstart);
            disp(strcat('Segmentation Time:',num2str(segend),' Seconds - MATLAB Worker No',num2str(s),' for',subname))
            logfile(logname,[subname '(' num2str(s) ') COMPLETED start_seg:' ' ' num2str(segend),' Seconds']);
        catch
            status = strcat('ERROR: SEGMENTAION FAIL ! - MATLAB Worker No',num2str(s),' for',subname);
            logfile(logname,[subname '(' num2str(s) ') FAILED: start_seg']);
            disp(status)
        end
    else
        disp('======================================================')
        ...
        disp('======================================================')
        disp(strcat('- MATLAB Worker No',num2str(s),' for',subname))
        logfile(logname,[subname '(' num2str(s) ') SKIPPED: start_seg']);
    end
    ...
    if (isempty(T2) && ~exist([dirname filesep baseFilenameRSPD '_T1orT2_masks.nii'],'file')) ||...
        ...
        disp('======================================================')
        disp(strcat('- MATLAB Worker No',num2str(s),' for',subname))
        logfile(logname,[subname '(' num2str(s) ')  BEGAN: segTouchup']);
        try
            segupstart = tic;
            segTouchup(subjRSPD,T2);
            segupend = toc(segupstart);
            disp(strcat('Touch Up Time:',num2str(segupend),' Seconds - MATLAB Worker No',num2str(s),' for',subname))
            logfile(logname,[subname '(' num2str(s) ') COMPLETED segTouchup:' ' ' num2str(segupend),' Seconds']);
        catch
            status = strcat('ERROR: TOUCHUP FAIL ! - MATLAB Worker No',num2str(s),' for',subname);
            disp(status)
            logfile(logname,[subname '(' num2str(s) ') FAILED: segTouchup']);
        end
    else
        disp('======================================================')
        ...
        disp('======================================================')
        disp(strcat('- MATLAB Worker No',num2str(s),' for',subname))
        logfile(logname,[subname '(' num2str(s) ') SKIPPED: segTouchup']);
    end
else
    disp('======================================================')
...
if ~exist([dirname filesep baseFilename '_' uniqueTag '_mask_elec.nii'],'file')
    ...
    disp('======================================================')
    disp(strcat('- MATLAB Worker No',num2str(s),' for',subname))
    logfile(logname,[subname '(' num2str(s) ') BEGAN: electrodePlacement']);
    try
        placestart = tic;
        hdrInfo = electrodePlacement(subj,subjRSPD,T2,elecName,options,uniqueTag);
        placeend = toc(placestart);
        disp(strcat('Placement Time:',num2str(placeend),' Seconds - MATLAB Worker No',num2str(s),' for',subname))
        logfile(logname,[subname '(' num2str(s) ') COMPLETED electrodePlacement:' ' ' num2str(placeend),' Seconds']);
    catch
        status = strcat('ERROR: PLACEMENT FAIL ! - MATLAB Worker No',num2str(s),' for',subname);
        logfile(logname,[subname '(' num2str(s) ') FAILED: electrodePlacement']);
        disp(status)
    end
else
    disp('======================================================')
    ...
    logfile(logname,[subname '(' num2str(s) ') SKIPPED: electrodePlacement']);
end
...
if ~exist([dirname filesep baseFilename '_' uniqueTag '.mat'],'file')
    ...
    disp('======================================================')
    disp(strcat('- MATLAB Worker No',num2str(s),' for',subname))
    logfile(logname,[subname '(' num2str(s) ') BEGAN: meshByIso2mesh']);
    try
        meshstart = tic;
        [node,elem,face] = meshByIso2mesh(s,subj,subjRSPD,T2,meshOpt,uniqueTag);
        meshend = toc(meshstart);
        disp(strcat('Mesh Time:',num2str(meshend),' Seconds - MATLAB Worker No',num2str(s),' for',subname))
        logfile(logname,[subname '(' num2str(s) ') COMPLETED meshByIso2mesh:' ' ' num2str(placeend),' Seconds']);
    catch
        status = strcat('ERROR: MESH FAIL ! - MATLAB Worker No',num2str(s),' for',subname);
        disp(status)
        logfile(logname,[subname '(' num2str(s) ') FAILED: meshByIso2mesh']);
    end
else
    disp('======================================================')
    ...
    logfile(logname,[subname '(' num2str(s) ') SKIPPED: meshByIso2mesh']);
end
...
if ~exist([dirname filesep baseFilename '_' uniqueTag '_v.pos'],'file')
    ...
    disp('======================================================')
    disp(strcat('- MATLAB Worker No',num2str(s),' for',subname))
    logfile(logname,[subname '(' num2str(s) ') BEGAN: solveByGetDP']);
    try
        getDPstart = tic;
        prepareForGetDP(subj,node,elem,hdrInfo,elecName,uniqueTag);
        solveByGetDP(subj,injectCurrent,conductivities,uniqueTag);
        getDPend = toc(getDPstart);
        disp(strcat('GetDP Time:',num2str(getDPend),' Seconds - MATLAB Worker No',num2str(s),' for',subname))
        logfile(logname,[subname '(' num2str(s) ') COMPLETED solveByGetDP:' ' ' num2str(getDPend),' Seconds']);
    catch
        status = strcat('ERROR: GET DP FAIL ! - MATLAB Worker No',num2str(s),' for',subname);
        disp(status)
        logfile(logname,[subname '(' num2str(s) ') FAILED: solveByGetDP']);
    end
else
    disp('======================================================')
    ...
    disp('======================================================')
    disp(strcat('- MATLAB Worker No',num2str(s),' for',subname))
    logfile(logname,[subname '(' num2str(s) ') SKIPPED: solveByGetDP']);
    %     load([dirname filesep baseFilename '_' uniqueTag '_elecMeshLabels.mat'],'label_elec');
end

if ~exist([dirname filesep baseFilename '_EV.mat'],'file')
    disp('======================================================')
    disp('STEP 6 (final step): SAVING AND VISUALIZING RESULTS...')
    disp('======================================================')
    disp(strcat('- MATLAB Worker No',num2str(s),' for',subname))
    logfile(logname,[subname '(' num2str(s) ') BEGAN: postGetDP']);
    try
        postGetDPstart = tic;
        [vol_all,ef_mag,ef_all] = postGetDP(subj,subjRSPD,node,hdrInfo,uniqueTag);
        postGetDPend = toc(postGetDPstart);
        disp(strcat('postGetDP Time:',num2str(postGetDPend),' Seconds - MATLAB Worker No',num2str(s),' for',subname))
        logfile(logname,[subname '(' num2str(s) ') COMPLETED postGetDP:' ' ' num2str(postGetDPend),' Seconds']);
        %     visualizeRes(subj,subjRSPD,T2,node,elem,face,vol_all,ef_mag,ef_all,injectCurrent,hdrInfo,uniqueTag,0);
    catch
        status = strcat('ERROR: POST GET DP FAIL ! - MATLAB Worker No',num2str(s),' for',subname);
        disp(status)
        logfile(logname,[subname '(' num2str(s) ') FAILED: postGetDP']);
    end
else
    disp('======================================================')
    ...
    disp('======================================================')
    disp(strcat('- MATLAB Worker No',num2str(s),' for',subname))
    logfile(logname,[subname '(' num2str(s) ') SKIPPED: postGetDP']);
...
if exist('segupend','var')
    Timing.SegUp = segupend;
    disp(strcat('Touch Up Time:',num2str(segupend),' Seconds - MATLAB Worker No',num2str(s),' for',subname))
end
if exist('placeend','var')
    Timing.Place = placeend;
    disp(strcat('Placement Time:',num2str(placeend),' Seconds - MATLAB Worker No',num2str(s),' for',subname))
end
if exist('meshend','var')
    Timing.Mesh = meshend;
    disp(strcat('Mesh Time:',num2str(meshend),' Seconds - MATLAB Worker No',num2str(s),' for',subname))
end
if exist('getDPend','var')
    Timing.GetDP = getDPend;
    disp(strcat('GetDP Time:',num2str(getDPend),' Seconds - MATLAB Worker No',num2str(s),' for',subname))
end
if exist('postgetDPend','var')
    Timing.PostDP = postGetDPend;
    disp(strcat('PostGetDP Time:',num2str(postGetDPend),' Seconds - MATLAB Worker No',num2str(s),' for',subname))
end
hiperoastrend = toc(hiperoastrstart);
Timing.Total = hiperoastrend;
save([dirname filesep baseFilename '_' savetag '_Timing.mat'],'Timing')
disp(strcat('TOTAL Hiperoastr Time:',num2str(hiperoastrend),' Seconds - MATLAB Worker No',num2str(s),' for',subname))
if exist(fullfile(dirname,'T1_Jroast.nii'),'file')
    status = 'SUCCESSFUL!';
    logfile(logname,[subname '(' num2str(s) ') ROAST SUCCESSFUL:' ' ' num2str(hiperoastrend),' Seconds']);
else
    status = 'Complete without Jroast';
end