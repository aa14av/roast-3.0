...
function exhaustiveSearch_v5(sub,e)
...
Nvox = numel(allMask); nd = length(activ)/Nvox;
brain = repmat(allMask(:) == 1 | allMask(:) == 2,3,1);
activ = activ(brain)./100; % shrink weights to size of group brain

% CREATE GMM WEIGHT VECTOR
%=====================================================================================
fprec = zeros(Nvox*nd,1);
fprec(brain(1:Nvox*nd)) = ...
    ( activ' .* (sum(activ ~= 0) / length(activ)) ) + ... % Scale SVM weights
    ( 1 - sum(activ' .* (sum(activ ~= 0) / length(activ))) ) ... % Distribute weight amongst remaining voxels
    / length(activ);
%=====================================================================================             
